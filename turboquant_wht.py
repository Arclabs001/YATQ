...
LM_BOUNDARIES = {
    1: torch.tensor([0.]),
    2: torch.tensor([-0.9816,  0.    ,  0.9816]),
    3: torch.tensor([-1.7479, -1.05  , -0.5005,  0.    ,  0.5005,  1.05  ,  1.7479]),
    4: torch.tensor([-2.4008, -1.8435, -1.4371, -1.0993, -0.7995, -0.5224, -0.2582,  0.    ,
  0.2582,  0.5224,  0.7995,  1.0993,  1.4371,  1.8435,  2.4008]),
    5: torch.tensor([-2.976 , -2.5045, -2.1733, -1.908 , -1.6818, -1.4813, -1.2991, -1.1303,
 ...
 -0.4889, -0.4178, -0.3473, -0.2773, -0.2077, -0.1383, -0.0691,  0.    ,
  ...
  2.1103,  2.2544,  2.4159,  2.6015,  2.8233,  3.1058,  3.5169]),
    7: torch.tensor([-4.0871, -3.7257, -3.4813, -3.292 , -3.1356, -3.0011, -2.8824, -2.7756,
 ...
 -0.2825, -0.2421, -0.2017, -0.1613, -0.121 , -0.0807, -0.0403,  0.    ,
  ...
  2.8824,  3.0011,  3.1356,  3.292 ,  3.4813,  3.7257,  4.0871]),
    8: torch.tensor([-4.502 , -4.1704, -3.9485, -3.7782, -3.6388, -3.52  , -3.4161, -3.3236,
...
LM_CENTROIDS = {
    ...
    3: torch.tensor([-2.1519, -1.3439, -0.756 , -0.2451,  0.2451,  0.756 ,  1.3439,  2.1519]),
    4: torch.tensor([-2.7326, -2.069 , -1.618 , -1.2562, -0.9423, -0.6568, -0.388 , -0.1284,
 ...
 -0.3027, -0.2623, -0.2219, -0.1815, -0.1412, -0.1008, -0.0605, -0.0202,
  ...
  2.8264,  2.9384,  3.0638,  3.2074,  3.3767,  3.5859,  3.8654,  4.3088]),
    8: torch.tensor([-4.7062, -4.2978, -4.043 , -3.854 , -3.7025, -3.5751, -3.4649, -3.3674,
...
def lloyd_max_quantize(x: torch.Tensor, bits: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    # Use precomputed Lloyd-Max if available, otherwise use uniform
    if bits in LM_BOUNDARIES:
        boundaries = LM_BOUNDARIES[bits].to(x.device)
        centroids = LM_CENTROIDS[bits].to(x.device)
    else:
        # Fallback to uniform quantization for unsupported bits
